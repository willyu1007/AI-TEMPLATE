...
.PHONY: help dev_check docgen ai_begin dag_check contract_compat_check \
        ...
        test_status_check dataflow_check app_structure_check cleanup_temp \
...
help:
	...
	@echo "  make cleanup_temp            - Remove temporary files"
	...
	@echo "  make agent_lint             - Lint AGENTS.md"
...
cleanup_temp:
	...
	@find . -type f -name "*_temp.*" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.venv/*" -not -path "./venv/*" -delete 2>/dev/null || true


	@find . -type d -name "*_temp" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.venv/*" -not -path "./venv/*" -exec rm -rf {} + 2>/dev/null || true


	@if [ -d "temp" ]; then \


		find temp -type f -name "*_temp.*" -delete 2>/dev/null || true; \
...
temp_files_check:
	...
	if find . -type f -name "*_temp.*" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.venv/*" -not -path "./venv/*" -not -path "./temp/*" 2>/dev/null | grep -q .; then \
		...
		find . -type f -name "*_temp.*" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.venv/*" -not -path "./venv/*" -not -path "./temp/*" 2>/dev/null; \


		echo ""; \


		echo "? make cleanup_temp"; \
...
cleanup_all: cleanup_temp cleanup_reports_smart
